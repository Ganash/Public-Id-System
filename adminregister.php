...
// basic admin id validation


if (empty($adid)) {
   $error = true;
   $mobileError = "* Please enter your Admin Id. *";
  } else if (strlen($mobile) < 5) {
   $error = true;
   $mobileError = "* Admin Id must have atleast 5 digits. *";
  } else if (!preg_match("/^[0-9]+$/",$mobile)) {
   $error = true;
   $mobileError = "* Admin Id must contain only digits. *";
  }
...
if (empty($mobile)) {
  ...
  if( !$error ) {
   ...
   $query = "INSERT INTO adminform(adminid,name,email,phonenumber,password) VALUES('$adid','$name','$email','$mobile','$pass')";
...
<form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off" enctype="multipart/form-data">
   ...
   ?>
        ...
        <input type="text" name="adid" class="inputFields"  maxlength="15" value="<?php echo @$_SESSION['adid'] ?>" required/><br/>
        <span class="text-danger"><?php echo $adidError; ?></span>
        </div>
        </div>
				...
				<input type="integer" name="mbile"  class="inputFields" maxlength="15" value="<?php echo @$_SESSION['mobile'] ?>"  required/><br/>
